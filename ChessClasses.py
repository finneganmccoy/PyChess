...
class Position:
    ...
    def change(self, newPosition):
        ...
        bobject.squares[oldPosition[0]][oldPosition[1]] = None
        bobject.squares[newPosition[0]][newPosition[1]] = self.parentPiece
        bobject.updateAll()
        for i in bobject.kings:
            if i.is_in_check == True and i.color == self.parentPiece.color:
                bobject.squares[newPosition[0]][newPosition[1]] = wasInSquare
                bobject.squares[oldPosition[0]][oldPosition[1]] = self.parentPiece
    ...
    def bishopMoves(self):
        ...
        for _ in range(2):
            for i in range(2):
                ...
                while bobject.colorCheck(moveAttempt) != self.color:
                    if bobject.colorCheck(moveAttempt) == "Out of bounds":
                        break
    ...
    def knightMoves(self):
        ...
        for _ in range(2):
            for _ in range(2):
                for _ in range(2):
                    moveAttempt = [currentCoords[0] + relativeCoords[0], currentCoords[1] + relativeCoords[1]]
                    if bobject.colorCheck(moveAttempt) != self.color and bobject.colorCheck(moveAttempt) != "Out of bounds":
                        moves.append(moveAttempt)
                    relativeCoords[0] *= -1
                relativeCoords[1] *= -1
            relativeCoords.reverse()
